...
_DEFAULT_VIDEOALIGN_ROOT = Path(__file__).resolve().parents[4] / "third_party" / "rl_rewards" / "VideoAlign"
_VIDEOALIGN_ROOT = Path(os.environ.get("VIDEOALIGN_RUNTIME_PATH") or os.environ.get("VIDEOALIGN_ROOT")
                        or _DEFAULT_VIDEOALIGN_ROOT)
    ...
    if cache_key not in _VIDEOALIGN_INFERENCERS:
        try:
            ...
            raise ImportError("VideoAlign rewards require the VideoAlign runtime files under "
                              "fastvideo/third_party/rl_rewards/VideoAlign or VIDEOALIGN_RUNTIME_PATH, "
                              "plus a VIDEOALIGN_CHECKPOINT_PATH checkpoint.") from exc
