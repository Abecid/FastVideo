"""Scientific launcher for finite-transition v2.

This wrapper uses ``modal_train_finite_transition_v2_run.train_v2`` only for
asset/config preparation, then replaces the method target with the final
``FiniteTransitionV2ScientificMethod`` and runs the job to completion.  Use this
file, not the earlier provisional v2 launcher.
"""
...
app = modal.App("fastvideo-finite-transition-v2-scientific")
...
def execute_scientific(
    ...
    *,
    ...
    raw["method"]["_target_"] = (
        "fastvideo.train.methods.rl.finite_transition_v2_scientific."
        "FiniteTransitionV2ScientificMethod"
    )
    raw["training"]["tracker"]["run_name"] = run_name
    scientific_path = config_path.with_name("resolved_v2_scientific.yaml")
    ...
    os.environ["WANDB_TAGS"] = (
        f"finite-transition-v2,scientific,{job_type}"
    )
    subprocess.run(
        ["bash", "examples/train/run.sh", str(scientific_path)],
        cwd=PROJECT_ROOT,
        check=True,
    )
    runs_volume.commit()
    cache_volume.commit()
    return {
        "run_config": str(scientific_path),
        "run_name": run_name,
...
def main(
    ...
    comparison_id = comparison_id.strip() or (
        f"ftv2_scientific_s{seed}_"
        f"{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
    )

    def prepare(preset_name: str, *, lr: float, steps: int, every: int, name: str):
    ...
    if lr_sweep:
        ...
        handles = [
            execute_scientific.spawn(
                item["run_config"],
                run_name=name,
                comparison_id=comparison_id,
                job_type=f"grpo_lr_{lr:.0e}",
            )
            for item, name, lr in prepared
        ]
    ...
    if paired:
        ...
        if errors:
            raise RuntimeError("Scientific v2 arm failure: " + " | ".join(errors))
        return
