"""Corrected Modal launcher for finite-transition v2 experiments.

This file supersedes ``modal_train_finite_transition_v2.py``. It reuses the
proven CUDA/FlashAttention image and persistent volumes from the original FTPP
launcher, merges the selected v2 scientific preset with resolved asset paths,
and blocks until every spawned comparison arm finishes or raises.
"""
...
app = modal.App("fastvideo-finite-transition-v2-run")
...
def train_v2(
    ...
    os.environ["WANDB_TAGS"] = (
        f"finite-transition-v2,anyflow,{preset},seed-{seed}"
    )
    ...
    resolved_data = generated["training"]["data"]["data_path"]
    resolved_validation = generated["method"]["validation"]["data_path"]
...
def main(
    ...
    if paired:
        ...
        for handle in handles:
            try:
                handle.get()
            except Exception as exc:  # noqa: BLE001 - surface both remote arms
                errors.append(repr(exc))
        if errors:
            raise RuntimeError("One or more v2 arms failed: " + " | ".join(errors))
        return
