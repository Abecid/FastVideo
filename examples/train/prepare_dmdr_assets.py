...
"""Prepare reproducible assets for DMDR Wan video RL training.

This script is intentionally environment-agnostic. It contains the prompt,
text-only preprocessing, VideoReward checkpoint, reward preflight, and run
config preparation needed to reproduce the DMDR video run without relying on a
private Modal launcher.
"""
...
DEFAULT_CONFIG = "examples/train/configs/rl/wan/dmdr_videoalign.yaml"
DEFAULT_OUTPUT_DIR = "outputs/wan2.1_dmdr_videoalign"

VIDEOALIGN_REWARD_NAMES = (
    "videoalign_vq",
    "videoalign_mq",
    "videoalign_ta",
)
...
def resolve_reward_map(reward: str) -> tuple[dict[str, float], str]:
    ...
    if reward in {"videoalign", "video_reward", "video_multi_reward"}:
        reward_map = {name: 1.0 for name in VIDEOALIGN_REWARD_NAMES}
    elif reward in {"videoalign_ta_only", "ta"}:
        reward_map = {"videoalign_ta": 1.0}
    elif reward in {"videoalign_quality", "quality"}:
        reward_map = {"videoalign_vq": 1.0, "videoalign_mq": 1.0}
    else:
        reward_map = {reward: 1.0}
    backend = "genrl" if any(name in GENRL_REWARD_NAMES for name in reward_map) else "auto"
...
def ensure_videoalign_checkpoint(path: Path) -> None:
    ...
    try:
        ...
        raise ImportError(f"huggingface_hub is required to download {VIDEO_REWARD_REPO}. "
                          "Install examples/train/requirements-dmdr.txt and rerun.") from exc
    snapshot_download(
...
def load_prompts(dataset: str) -> tuple[list[str], str]:
    ...
    if dataset in {"world-r1", "world-r1-final", "world_r1", "world_r1_final"}:
        ...
        if not source.is_file():
            raise RuntimeError(f"Dataset {dataset!r} is not a built-in World-R1 alias or prompt text file.")
...
def prepare_prompt_file(args: argparse.Namespace) -> tuple[Path, Path, int, str]:
    prompts, source_label = load_prompts(args.dataset)
    ...
    dataset_root = args.data_root / f"dmdr_{args.dataset}_text_only_f{args.num_frames}_{prompt_suffix}"
    parquet_dir = dataset_root / "combined_parquet_dataset"
    prompt_file = args.data_root / "prompts" / f"dmdr_{args.dataset}_{prompt_suffix}_train.txt"
...
def write_run_config(
    ...
    *,
    ...
    validation_config["batch_size"] = int(args.validation_batch_size)
    ...
    run_config_path = args.run_config_dir / "dmdr_wan_run.yaml"
...
def check_reward_runtime(
    ...
    *,
    ...
    print("=== DMDR reward preflight ===", flush=True)
    ...
    print("=== DMDR reward preflight OK ===", flush=True)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data-root", type=Path, default=Path("data/dmdr"))
    parser.add_argument("--cache-root", type=Path, default=Path(".cache/dmdr"))
    parser.add_argument("--output-dir", type=Path, default=Path(DEFAULT_OUTPUT_DIR))
    parser.add_argument("--run-config-dir", type=Path, default=Path("outputs/dmdr_run_configs"))
    ...
    parser.add_argument("--num-batches-per-epoch", type=int, default=16)
    ...
    parser.add_argument("--validation-every-steps", type=int, default=5)
    parser.add_argument("--validation-num-steps", type=int, default=30)
    parser.add_argument("--validation-num-prompts", type=int, default=8)
    parser.add_argument("--validation-batch-size", type=int, default=1)
    parser.add_argument("--log-sample-max-videos", type=int, default=8)
    parser.add_argument("--project-name", default="dmdr_wan")
...
def main() -> None:
    ...
    prompt_file, parquet_dir, prompt_count, source_label = prepare_prompt_file(args)
