...
"""Compare FastVideo VideoAlign scores against an upstream reference JSON.

Create the reference once in an upstream-supported VideoAlign environment using
the same videos/prompts, then run this script inside the FastVideo/Modal runtime.
The check validates absolute score drift and rank correlation for MQ, VQ and TA.
"""
...
def main() -> None:
    ...
    scorers = {
        "MQ": VideoAlignMotionQualityScorer(device=args.device),
        "VQ": VideoAlignVisualQualityScorer(device=args.device),
        "TA": VideoAlignTextAlignmentScorer(device=args.device),
    }
    rows = []
    ...
    if args.reference is not None:
        ...
        comparisons = {}
        for name in scorers:
            current_values = np.asarray([row[name] for row in rows], dtype=np.float64)
            reference_values = np.asarray(
                [float(row[name]) for row in reference_rows],
                dtype=np.float64,
            )
            max_error = float(np.max(np.abs(current_values - reference_values)))
            rank_corr = _pearson(_rank(current_values), _rank(reference_values))
            comparisons[name] = {
            ...
            if max_error > args.max_abs_error:
                failures.append(
                    f"{name} max abs error {max_error:.6g} > {args.max_abs_error:.6g}"
                )
    ...
    if failures:
        raise RuntimeError("VideoAlign calibration parity failed: " + "; ".join(failures))
