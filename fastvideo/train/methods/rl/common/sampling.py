...
class SamplingConfig:
    ...
    def from_mapping(cls, raw: dict[str, Any] | None) -> SamplingConfig:
        ...
        num_steps_raw = raw.get("num_steps", 25)
        num_steps = 25 if num_steps_raw is None else int(num_steps_raw)
        if num_steps <= 0:
            raise ValueError("method.sampling.num_steps must be positive")
        guidance_raw = raw.get("guidance_scale", 1.0)
        guidance_scale = 1.0 if guidance_raw is None else float(guidance_raw)
