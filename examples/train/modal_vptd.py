...
"""Launch Video Posterior Transition Distillation on Modal.
...
preflight, logs the untouched four-step base model, and only then begins VPTD.
...
Examples:
    ...
    modal run examples/train/modal_vptd.py --max-steps 50
    ...
    modal run examples/train/modal_vptd.py --max-steps 1200
...
DEFAULT_BRANCH = "agent/video-posterior-transition-distillation"
...
DEFAULT_CONFIG = "examples/train/configs/vptd/wan_anyflow_videoalign_mq.yaml"
...
GPU = os.environ.get("VPTD_MODAL_GPU", "H100:4")
SECRET_NAME = os.environ.get("VPTD_MODAL_SECRET", "fastvideo-training")
...
app = modal.App("fastvideo-vptd")
hf_cache = modal.Volume.from_name("fastvideo-vptd-hf-cache", create_if_missing=True)
runs = modal.Volume.from_name("fastvideo-vptd-runs", create_if_missing=True)
...
def train(
    *,
    ...
    max_steps: int = 50,
    objective: str = "posterior_distillation",
    reward_key: str = "videoalign_mq",
    run_name: str = "wan_anyflow_vptd_videoalign_mq",
    resume: str = "auto",
    preflight_only: bool = False,
    smoke: bool = False,
) -> dict[str, str | int | bool]:
    """Run one resumable VPTD target on four H100s.
    ...
    and the method-vs-Flow-Map-GRPO ablation.
    """

    if max_steps <= 0:
        raise ValueError("max_steps must be positive")
    if objective not in {"posterior_distillation", "flowmap_grpo"}:
        raise ValueError("objective must be posterior_distillation or flowmap_grpo")
    ...
    env.update(
        {
            ...
            "VIDEOALIGN_CHECKPOINT_PATH": "/runs/cache/vptd/VideoReward",
    ...
    try:
        ...
        _run(
            [
                ...
                "--system",
                "-r",
                "examples/train/requirements-vptd.txt",
            ],
            cwd=fastvideo,
            env=env,
        )
        ...
        _run(
            [
                ...
                "pytest",
                "-q",
                "fastvideo/tests/train/methods/test_vptd_core.py",
            ],
            cwd=fastvideo,
            env=env,
        )
        _run(
            [
                ...
                (
                    ...
                    "build_multi_reward_scorer; print('VPTD imports OK')"
                ),
            ],
            cwd=fastvideo,
            env=env,
        ...
        command = [
            ...
            "--master_port",
            env["MASTER_PORT"],
            "examples/train/vptd/train_anyflow.py",
            "--config",
            DEFAULT_CONFIG,
            "--output-dir",
            str(output_dir),
